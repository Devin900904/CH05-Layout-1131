...
public class M0500 extends AppCompatActivity {
    ...
    private View.OnClickListener b001on = new View.OnClickListener() {         //private View開頭 宣告b001on按鈕被觸碰後要執行的動作
        ...
        public void onClick(View v) {                                                              // DecimalFormat   pondsFormat = new DecimalFormat("0.00000");   ---複製用
            pondsFormat = new DecimalFormat("0.0000000");         //複製上面的刪掉DecimalFormat後 pondsFormat選local
            if (e001.getText().toString().length() > 0) {
                ans = pondsFormat.format(Float.parseFloat(e001.getText().toString()) * 0.0003408);//紅的"ans"選local 然後選"String"  (number:"隨意打")
                t003.setText(ans);
                ans = pondsFormat.format(Float.parseFloat(e001.getText().toString()) * 3.3058);
                t005.setText(ans);
                ans = pondsFormat.format(Float.parseFloat(e001.getText().toString()) * 0.033058);
                t007.setText(ans);
